...
export default function withKeepAliveContextConsumer<P = any>(
  Component: React.ComponentType<IKeepAliveContextConsumerComponentProps & P>,
) {
  const WithKeepAliveContextConsumer = (props: P) => (
    <KeepAliveContext.Consumer>
      {(contextProps: IKeepAliveContextProps) => (
        <Component _keepAliveContextProps={contextProps} {...props} />
      )}
    </KeepAliveContext.Consumer>
  );

  WithKeepAliveContextConsumer.displayName = `${WithKeepAliveContextConsumerDisplayName}(${getDisplayName(
    Component,
  )})`;
